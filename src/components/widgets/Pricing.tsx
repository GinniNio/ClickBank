...
const Pricing = ({ header, prices, id, hasBackground = false }: PricingProps) => (
  <WidgetWrapper id={id ? id : ''} hasBackground={hasBackground} containerClass="">
    ...
    <div className="flex items-stretch justify-center">
      <div className="grid grid-cols-1 gap-3 dark:text-white sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
        {prices &&
          prices.map(
            ({ title, subtitle, price, period, items, callToAction, hasRibbon = false, ribbonTitle }, index) => (
              ...
              >
                {price && period && (
                  <div className="card max-w-sm flex flex-col justify-between text-center relative">
                        ...
                        <div className="my-8 md:my-10 space-y-2 text-left">
                          {items.map((item, index) => (
                            <div key={`item-${index}`} className="flex items-start">
                              <IconCheck className="w-4 h-4 mt-1.5 mr-3 rtl:mr-0 rtl:ml-3 flex items-center justify-center rounded-full border-2 border-primary-600 bg-primary-600 text-white dark:text-slate-200" />
                              <div>
